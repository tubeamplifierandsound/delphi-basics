...
Var
  ...
  Ind, Count, CurrN, i, j, ASNum, ElNum: Integer;
  Flag: Boolean;
  El: String[255];
  dot: Byte;
{
 ...
 ASNum - the code of the element in the ASCII table;
 ElNum - the number of the element in the line;
 Flag - a logical variable for checking the correctness of the input;
 El - a string to store the entered value;
 dot - variable to exclude the repetition of the separator sign in the entered value
}

Begin
  Writeln('The real number must be represented as a fixed point, the separating sign is a comma ","');

  // Entering array elements and checking the entered values
  For i := 1 to N do
  Begin
    For j := 1 to N do
    Begin
      
      // Assigning a false value to a boolean variable in order to enter the cycle of checking the entered value
      Flag := False;
      
      // Cycle for checking the entered value for correctness
      While Flag = False do
      Begin
        Write('X[', i, ',', j, '] = ');
        
        // Initialization of validation loop variables
        // The entered value is assigned to the string, since the user may enter an incorrect value
        Readln(El);
        ElNum := 1;
        dot := 0;

        // The check will go character by character to the last character of the string
        While ElNum <= Length(El) do
        Begin
          Flag := False;
          ASNum := Ord(El[ElNum]);

          // The first character can be either a digit or a minus sign
          If ElNum = 1 then
          Begin
            ...
            Else
              ...
              // Assigning a true value to a boolean variable in order to move to another character, since the character is entered correctly
              Flag := True
          End
          
          // Checking the remaining characters of the string
          Else
          Begin
            
            // If the character is the last one in the string, then it can only be a digit
            ...
            // Characters other than the first and last can be either digits, or one of them can be a separating comma
            Else
            Begin
              If ((ASNum < 48) or (ASNum > 57)) then
              Begin
                ...
                Begin
                  ...
                  // Checking for the uniqueness of the dividing sign
                  If dot = 0 then
                  Begin
                    Flag := True;
                    dot := dot+1;
                  ...
                  // Error in case of multiple delimiter characters in a string
                  Else
                  Begin
                    Writeln('Incorrect input!');
                    Writeln('Elements of the matrix must be of real type');
                ...
                // Conclusion about incorrect input
                Else
                Begin
                  Writeln('Incorrect input!');
                  Writeln('Elements of the matrix must be of real type');
              ...
              Else
                ...
                // Assigning a true value to a variable, since the value was entered correctly
                Flag := True;
            End;
          End;
          If Flag = True then
          ...
          Begin
            ...
            Writeln('The separating sign in real numbers is a ","');
            
            // Actions to exit the character-by-character verification cycle
            ElNum := Length(El)+1;
          End;
        End;
      End;
      
      // In case of correct input, the value is assigned to the element of the original array
      X[i,j] := StrToFloat(El);
    End;
    Writeln;
  End;
